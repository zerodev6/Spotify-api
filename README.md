...
variables are required for the default configuration. The repository also
includes `Procfile` and `start.sh` for a Python buildpack deployment. If you
use the buildpack instead of Docker, set the run command to:

```bash
sh start.sh
```

After deployment, verify the service itself before testing YouTube:

```bash
curl https://YOUR-KOYEB-DOMAIN/health
curl https://YOUR-KOYEB-DOMAIN/
curl "https://YOUR-KOYEB-DOMAIN/api/search?q=song"
```

The first command must return `{"status":"ok","service":"youtube-music-api"}`.
If `/health` or `/` returns 404, Koyeb is running a different service,
directory, branch, or start command—not this FastAPI application. Make sure
the service root is the repository root and the listening port is `8000`.
